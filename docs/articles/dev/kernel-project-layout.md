...
| **Cosmos.Kernel.HAL.Interfaces** | Pure interfaces, no implementations. The device contracts a kernel supplies drivers against (`IBlockDevice`, `INetworkDevice`, `IKeyboardDevice`, ...) are public; the boot contract `IPlatformInitializer` and `IGraphicDevice` are internal. |
