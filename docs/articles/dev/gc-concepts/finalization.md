...
A finalizer (`~ClassName` in C#) runs cleanup code before an object's memory is reclaimed, usually to release a native resource. Finalization changes the collector's job: an unreachable object with a pending finalizer cannot be freed yet, so the GC must queue it, let a dedicated thread run the finalizer, and only reclaim the memory in a later collection ([Finalizers](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/finalizers)).
