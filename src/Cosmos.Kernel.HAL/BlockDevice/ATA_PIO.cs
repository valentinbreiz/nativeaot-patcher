...
public class ATA_PIO : Ata
{
    ...
    public ATA_PIO(AtaIoGroup aIO, ControllerIdEnum aControllerId, BusPositionEnum aBusPosition)
    {
        IO = aIO;
        mControllerID = aControllerId;
        mBusPosition = aBusPosition;
        // Disable IRQs, we use polling currently
        PlatformHAL.PortIO.WriteByte((ushort)IO.Control, 0x02);
    ...
    public SpecLevel DiscoverDrive()
    {
        ...
        if (PlatformHAL.PortIO.ReadByte((ushort)IO.Status) == 0xFF)
        {
            return SpecLevel.Null;
        }

        Status xIdentifyStatus = SendCmd(Cmd.Identify, false);
        ...
        else if ((xIdentifyStatus & Status.Error) != 0)
        {
            ...
            int xTypeId = (PlatformHAL.PortIO.ReadByte((ushort)IO.Lba2) << 8) | PlatformHAL.PortIO.ReadByte((ushort)IO.Lba1);
            if (xTypeId is 0xEB14 or 0x9669)
            {
    ...
    {
        PlatformHAL.PortIO.WriteByte((ushort)IO.DeviceSelect,
            (byte)((byte)(DvcSelVal.Default | DvcSelVal.LBA |
                          (mBusPosition == BusPositionEnum.Slave ? DvcSelVal.Slave : 0)) | aLbaHigh4));
        Wait();
    }
    ...
    /// <returns></returns>
    public Status SendCmd(Cmd aCmd) => SendCmd(aCmd, true);
    ...
    {
        PlatformHAL.PortIO.WriteByte((ushort)IO.Command, (byte)aCmd);
        Status xStatus;
        do
        {
            Wait();
            xStatus = (Status)PlatformHAL.PortIO.ReadByte((ushort)IO.Status);
    ...
    protected void InitDrive()
    {
        ...
        ushort[] xBuff = new ushort[256];
        for (int i = 0; i < xBuff.Length; i++)
        {
            xBuff[i] = PlatformHAL.PortIO.ReadWord((ushort)(IO.Lba2 + i));
        }
    ...
    protected void SelectSector(ulong aSectorNo, ulong aSectorCount)
    {
        ...
        if (LBA48Bit)
        {
            PlatformHAL.PortIO.WriteWord((ushort)IO.SectorCount, (ushort)aSectorCount);
            PlatformHAL.PortIO.WriteByte((ushort)IO.Lba0, (byte)(aSectorNo >> 24));
            PlatformHAL.PortIO.WriteByte((ushort)IO.Lba0, (byte)aSectorNo);
            PlatformHAL.PortIO.WriteByte((ushort)IO.Lba1, (byte)(aSectorNo >> 32));
            PlatformHAL.PortIO.WriteByte((ushort)IO.Lba1, (byte)(aSectorNo >> 8));
            PlatformHAL.PortIO.WriteByte((ushort)IO.Lba2, (byte)(aSectorNo >> 40));
            PlatformHAL.PortIO.WriteByte((ushort)IO.Lba2, (byte)(aSectorNo >> 16));
        }
        else
        {
            // Number of sectors to read
            PlatformHAL.PortIO.WriteByte((ushort)IO.SectorCount, (byte)aSectorCount);
            PlatformHAL.PortIO.WriteByte((ushort)IO.Lba0, (byte)aSectorNo);
            PlatformHAL.PortIO.WriteByte((ushort)IO.Lba1, (byte)(aSectorNo >> 8));
            PlatformHAL.PortIO.WriteByte((ushort)IO.Lba2, (byte)(aSectorNo >> 16));
    ...
    public override void ReadBlock(ulong aBlockNo, ulong aBlockCount, ref byte[] aData)
    {
        ...
        {
            aData[i] = PlatformHAL.PortIO.ReadByte((ushort)(IO.Data + i));
        }
    }
    ...
    public override void WriteBlock(ulong aBlockNo, ulong aBlockCount, ref byte[] aData)
    {
        ...
        for (int i = 0; i < aData.Length; i++)
        {
            PlatformHAL.PortIO.WriteByte((ushort)(IO.Data + i), aData[i]);
            Wait();
        }
    ...
    /// </summary>
    /// <returns></returns>
    public override string ToString() => "AtaPio";
}
