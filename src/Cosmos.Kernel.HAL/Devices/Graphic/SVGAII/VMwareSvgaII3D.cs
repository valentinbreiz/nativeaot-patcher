...
namespace Cosmos.Kernel.HAL.Devices.Graphic
{
    public unsafe class VMWareSVGAII3D
    {
        ...
        {
            device = PciManager.GetDevice(Pci.Enums.VendorId.VmWare, Pci.Enums.DeviceId.SvgaiiAdapter)!;
            if (device is null)
            {
        ...
        {
            device.WriteRegister32((byte)IOPortOffset.Index,(uint)register);
            device.WriteRegister32((byte)IOPortOffset.Value,value);
        }
        ...
        {
            device.WriteRegister32((byte)IOPortOffset.Index,(uint)register);
            return device.ReadRegister32((byte)IOPortOffset.Value);
        }
