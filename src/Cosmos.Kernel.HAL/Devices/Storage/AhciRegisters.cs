...
public class HbaCommandHeader
{
    ...
    /// <summary>Size of a 32-bit dword in bytes, used to step through AHCI structures when clearing them. Internal so the sibling AHCI structure classes share it.</summary>
    internal const int DwordSizeBytes = 4;
...
public class HbaCommandTable
{
    ...
    public HbaCommandTable(ulong address, uint prdtCount)
    {
        ...
        for (int i = 0; i < PrdtOffsetBytes / HbaCommandHeader.DwordSizeBytes; i++)
        {
            Native.MMIO.Write32(address + (ulong)(i * HbaCommandHeader.DwordSizeBytes), 0);
        }
...
public class HbaPrdtEntry
{
    ...
    public HbaPrdtEntry(ulong baseAddress, uint entry)
    {
        ...
        {
            Native.MMIO.Write32(_address + (ulong)(i * HbaCommandHeader.DwordSizeBytes), 0);
        }
    }
