...
public static unsafe partial class GarbageCollector
{
    ...
    /// <summary>
    /// Scans frozen segments for GC roots.
    /// Frozen objects are pre-initialized and should be treated as permanent roots.
    /// </summary>
    private static void ScanFrozenSegments()
    {
        FrozenSegmentInfo* segment = _frozenSegments;
        while (segment != null)
        {
            ScanFrozenSegment(segment);
            segment = segment->Next;
        }
    }

    /// <summary>
    /// Scans a single frozen segment for GC roots.
    /// </summary>
    private static void ScanFrozenSegment(FrozenSegmentInfo* segment)
    {
        // Frozen segments contain pre-initialized objects that may have references
        // We need to scan them for pointers to GC objects
        nint ptr = (nint)Align((uint)segment->Start);
        var end = segment->Start + (nint)Align((uint)segment->CommitSize);
        ...
        while(ptr < end)
        {
            ...
            {
                nint refPtr = PopMarkStack();
                GCObject* refObj = (GCObject*)refPtr;

                // Validate MethodTable - must point outside heap (to kernel code)
                nuint mtPtr = (nuint)refObj->MethodTable & ~(nuint)1;
                ...
                MethodTable* refMT = obj->GetMethodTable();
                if (refMT->ContainsGCPointers)
                    EnumerateReferences(refObj, refMT);
            }

            ptr = (nint)(ptr + Align(obj->ComputeSize()));
        }
    }
