...
public class IcmpTestServer : IDisposable
{
    ...
    private static readonly byte[] s_hostMac = { 0x52, 0x54, 0x00, 0x12, 0x34, 0x99 };
    private static readonly byte[] s_hostIp = { 10, 0, 2, 99 };
    private static readonly byte[] s_guestIp = { 10, 0, 2, 15 }; // slirp's first DHCP address
    private static readonly byte[] s_broadcastMac = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
    ...
    private void HandleArp(byte[] frame)
    {
        ...
        if (senderIp.SequenceEqual(s_guestIp) && _guestMac == null)
        {
        ...
        if (operation == 1 && targetIp.SequenceEqual(s_hostIp))
        {
    ...
    private void HandleIpv4(byte[] frame)
    {
        ...
        if (protocol != 1 || !destIp.SequenceEqual(s_hostIp))
        {
            return;
        }
    ...
    private static byte[] BuildArpRequest()
    {
        ...
        WriteEthernetHeader(frame, s_broadcastMac, 0x0806);
        WriteArpBody(frame, 1, s_hostMac, s_hostIp, new byte[6], s_guestIp);
        return frame;
    }
    ...
    private static byte[] BuildArpReply(byte[] targetMac, byte[] targetIp)
    {
        ...
        WriteArpBody(frame, 2, s_hostMac, s_hostIp, targetMac, targetIp);
        return frame;
    }
    ...
    private static void WriteEthernetHeader(byte[] frame, byte[] destMac, ushort etherType)
    {
        ...
        Array.Copy(s_hostMac, 0, frame, 6, 6);
    ...
    private byte[] BuildEchoRequest(byte[] guestMac, ushort sequence, byte[] payload)
    {
        ...
        frame[24] = 0x00;
        frame[25] = 0x00;
        Array.Copy(s_hostIp, 0, frame, 26, 4);
        Array.Copy(s_guestIp, 0, frame, 30, 4);
